...
void read_sco_file(FILE *file, sco_file_t *dest)
{
	...
	dest->mission_objects = malloc(dest->num_mission_objects * sizeof(mission_object_t));
	...
	if (dest->version >= 4)
	{
		...
		dest->ai_mesh = malloc(sizeof(ai_mesh_t));

		read_int(file, &dest->ai_mesh->num_vertices);
		dest->ai_mesh->vertices = malloc(dest->ai_mesh->num_vertices * sizeof(ai_mesh_vertex_t));
		...
		dest->ai_mesh->edges = malloc(dest->ai_mesh->num_edges * sizeof(ai_mesh_edge_t));
		...
		dest->ai_mesh->faces = malloc(dest->ai_mesh->num_faces * sizeof(ai_mesh_face_t));
	...
	if (ftell(file) != file_size)
	{
		...
		dest->ground_paint = malloc(sizeof(ground_paint_t));

		read_int(file, &dest->ground_paint->num_layers);
		dest->ground_paint->layers = malloc(dest->ground_paint->num_layers * sizeof(ground_paint_layer_t));
		...
		for(lcv = 0; lcv < dest->ground_paint->num_layers; ++lcv)
		{
			...
			if(has_cells)
			{
				dest->ground_paint->layers[lcv].continuity_count = malloc((dest->ground_paint->size_x * dest->ground_paint->size_y + 1) * sizeof(int));
				dest->ground_paint->layers[lcv].cells = malloc(dest->ground_paint->size_x * dest->ground_paint->size_y * sizeof(float));
